...
if __name__ == "__main__":
    ...
    class TestTree(unittest.TestCase):
        ...
        def test_performance(self):
            ...
            print(("{0:7d}|{2:9d}|{1.level:11d}|      0|{3:10.2f}|{4:10.1f}".format(qsize, tree, tsize,
                                                                                    float(tree.count) / qsize, e)))
            ...
            for p in qpos:
                node, distsq = tree.nearest(p)
            e2 = time() - s
            print(("{0:7d}|{2:9d}|{1.level:11d}|      0|{3:10.2f}|{4:10.1f}".format(qsize, tree, tsize * 10,
                                                                                    float(tree.count) / qsize, e2)))
            ...
            e3 = time() - s
            print(("{0:7d}|{2:9d}|{1.level:11d}|{5:7d}|{3:10.2f}|{4:10.1f}".format(qsize, tree, tsize * 10,
                                                                                   float(tree.count) / qsize, e3,
                                                                                   tsize * 10 // emptyq)))
